 ...
 * 电流峰值只有 2*pi*50*9.9uF*19.6V = 0.061A —— 误差比信号还大。该误差被内环 Kp
 * 成比例放大进占空比, 逼得 Kp 只能取小值 (现 2.2 欧, 而 Z0=sqrt(L/C)=14.21欧,
 * zeta=0.077), 空载失去负载阻尼后即振荡。
 * 注: C 由 12uF 改 9.9uF 后基波电流更小(0.074->0.061A), 纹波/信号比变差,
 *     对称点采样的正确性因此更关键, 但 L 不变 -> di/dt 不变 -> 残余误差仍约 8.6mA。
 *
 ...
 * 空载 di/dt=v_c/L=9800A/s -> 约 8.6mA, 远小于原方案的 0.15A。
 *
