...
ST_PR PR_Volt_PhaseA={.fpDes=0,.fpFB= 0,						 /*Des, FB*/
						  ...
						  //穿越点被推到1250~1400Hz, 对象相位-154度叠加75us延时(20kHz时)-38度
						  //= -192度 -> 相位裕度为负, 空载必振(带载时负载电阻压低谐振峰才稳)。
						  //(10kHz下延时150us, 同样场景会更差; 现值下LC区不穿越, 故不受影响)
...
//
//!! 20kHz->10kHz 后这一项的实际效果减半, 但故意不改 !!
//  控制延时 1.5*Ts (采样->算->下个counter=0装载->居中脉冲电压重心, 见hrtim.h)
//  从 75us 变 150us, 在 f_LC=1131Hz 处相位滞后 30.5deg -> 61.1deg。
//  延时使虚拟电阻变成复阻抗: 只有 cos(theta) 分量真正阻尼, sin 分量是电抗(略推高
//  谐振频率约6%)。故 zeta_eff = 0.0774*cos(theta): 20kHz 时 0.0667, 10kHz 时 0.0374,
//  物理Q值从 7.5 涨到 13.4 —— 扰动(死区/负载突变)后振铃更长, 但:
//   (1) 不威胁稳定性。下面那条推导证明 |L(w_LC)|=|PR|*Z0, Kp_i 精确约掉,
//       外环裕度与 Kp_i 无关(10kHz下 0.3495/9.13dB, 略优于20kHz的 0.3591/8.90dB)。
//   (2) 想补回 zeta 需 Kp_i 约 3.9, 但"阻尼变反阻尼"边界 f=1/(6*Ts) 从 3333Hz
//       掉到 1667Hz, 距 f_LC 只剩1.47倍。盲目加大会在 1.67~5kHz 段放大扰动。
//  结论: 保持 2.2 上机, 若示波器看到谐振振铃明显变长, 再从 3.0 起小步试。
 ...
 *     !! 峰窄到 ±0.32Hz 意味着离散化的峰位偏移不可忽略 !! 这是 10kHz 下必须
 *     做 Tustin 预畸变的原因(不做则 |H7| 掉到 1.05)。
 *   Kr=4.8 —— 见control.c的HARMONIC_KR。两个谐振器合计把 |L(w_LC)| 从
 *     基波单独的 0.2707(11.35dB) 推到 0.3495(9.13dB)。8dB是自设底线。
 *     (20kHz时对应 0.2773->0.3591, 即 11.14dB->8.90dB; 降频后两者均略好)
 *     谐振峰: |H5|@250Hz = |H7|@350Hz = Kp+Kr = 4.8 (10kHz + Tustin预畸变后)。
 *     对比: 20kHz无预畸变时因数字峰位偏移只有 4.45/3.21 —— 预畸变把这部分
 *     离散化损失补回来了, 详见 control.c 的 PR_PrewarpOmega。
