...
ST_PR PR_Volt_PhaseA={.fpDes=0,.fpFB= 0,						 /*Des, FB*/
						  ...
						  //穿越点被推到1250~1400Hz, 对象相位-154度叠加75us延时(20kHz)-38度
						  //= -192度 -> 相位裕度为负, 空载必振(带载时负载电阻压低谐振峰才稳)。
						  //现值下LC区不穿越, 故对延时不敏感(10kHz的150us延时也不破)。
...
//取 Kp_i=2.2 -> 等效阻尼电阻2.2欧, zeta=Kp/(2*Z0)=0.0774(无延时理想值)。
//
//!! 这一项的实际效果强烈依赖开关频率, 这是退回 20kHz 的主要理由 !!
//  控制延时 1.5*Ts (采样->算->下个counter=0装载->居中脉冲电压重心, 见hrtim.h)
//  20kHz: 75us  -> f_LC=1131Hz 处相位滞后 30.5deg
//  10kHz: 150us -> 同处滞后 61.1deg
//  延时使虚拟电阻变成复阻抗: 只有 cos(theta) 分量真正阻尼, sin 分量是电抗(略推高
//  谐振频率约6%)。故 zeta_eff = 0.0774*cos(theta):
//      20kHz -> 0.0667 (物理Q约 7.5)
//      10kHz -> 0.0374 (物理Q约 13.4, 扰动后振铃明显更长)
//  在 10kHz 下想把 zeta 补回来需 Kp_i 约 3.9, 但"阻尼变反阻尼"边界 f=1/(6*Ts)
//  同时从 3333Hz 掉到 1667Hz, 距 f_LC 只剩 1.47 倍(20kHz 时是 2.95 倍) ——
//  加大 Kp_i 会在 1.67~5kHz 段放大扰动。即 10kHz 下这个补偿无路可走,
//  而退回 20kHz 则 Kp_i 一个字都不用改, zeta_eff 自动回到 0.0667。
//  注: 外环裕度与 Kp_i 无关(下面推导证明 |L(w_LC)|=|PR|*Z0, Kp_i 精确约掉),
//      所以上面讨论的全是物理谐振峰的阻尼, 不是稳定性。
 ...
 *     该残留 ≈ 2*Kr*omiga_c/w, 与 omiga_0 几乎无关 -> 预畸变不动它。
 *     窄峰可行的前提: 谐波频率由开环角度发生器精确给出(无PLL误差),
 *     且 Control_SetFundamentalFreq 会随基波频率重算系数。
 *     !! 峰窄到 ±0.32Hz 意味着离散化的峰位偏移不可忽略 !! 这是必须做 Tustin
 *     预畸变的原因。不做的话峰位偏移随 Ts^2 增长:
 *         20kHz 无预畸变 -> |H5|=4.45, |H7|=3.21 (仍可用)
 *         10kHz 无预畸变 -> |H5|=2.52, |H7|=1.05 (7次谐振器基本失效)
 *     预畸变后两个频率下都精确等于 Kp+Kr。
 *   Kr=4.8 —— 见control.c的HARMONIC_KR。两个谐振器合计把 |L(w_LC)| 从
 *     基波单独的 0.2773(11.14dB) 推到 0.3591(8.89dB)。8dB是自设底线。
 *     (10kHz时对应 0.2707->0.3495, 即 11.35dB->9.13dB —— 略好, 因为双线性变换把
 *      1131Hz 映到的等效模拟频率 w_a 从 7182 升到 7422 rad/s, 而残留 ∝ 1/w_a。
 *      这 0.24dB 是退回 20kHz 的唯一代价, 换来 zeta_eff 0.0374->0.0667。)
 *     谐振峰: |H5|@250Hz = |H7|@350Hz = Kp+Kr = 4.8 (Tustin预畸变后)。
 *     详见 control.c 的 PR_PrewarpOmega。
