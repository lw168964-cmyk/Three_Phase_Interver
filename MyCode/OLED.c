...
void OLED_WriteCommand(uint8_t Command)
{
	...
	Cmd[0] = 0x00;
	Cmd[1] = Command;
	HAL_I2C_Master_Transmit(&hi2c1, 0x78, Cmd , sizeof(Cmd)/sizeof(Cmd[0]), HAL_MAX_DELAY);
}
...
/**
  ...
  * 返 回 值：无
  */
static uint8_t OLED_I2C_DataBuf[129];

static HAL_StatusTypeDef OLED_I2C_WaitReady(uint32_t Timeout)
{
...
{
    if ((Data == NULL) || (Count == 0) || (Count > 128))
    {
        return;
    }

    OLED_I2C_DataBuf[0] = 0x40;
    memcpy(&OLED_I2C_DataBuf[1], Data, Count);

    if (OLED_I2C_WaitReady(100) != HAL_OK)
    {
        return;
    }

    if (HAL_I2C_Master_Transmit_DMA(&hi2c1, 0x78, OLED_I2C_DataBuf, Count + 1) != HAL_OK)
    {
        /* DMA传输失败时退回到阻塞模式 */
        HAL_I2C_Master_Transmit(&hi2c1, 0x78, OLED_I2C_DataBuf, Count + 1, HAL_MAX_DELAY);
    }

    OLED_I2C_WaitReady(100);
}
